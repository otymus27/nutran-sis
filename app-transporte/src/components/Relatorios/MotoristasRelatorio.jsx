...
// Colunas disponíveis para motoristas
const allColumns = [
  { key: 'nome', label: 'Nome' },
...
const MotoristasRelatorio = ({ motoristas, loading }) => {
  ...
  const prepararDadosRelatorio = () => {
    let dadosFiltered = motoristas;

    // Se quiser, aqui pode colocar filtros de motoristas (exemplo: ativos/inativos)
    ...
    return dadosFiltered.map((motorista) => {
      const dados = {};
      allColumns.forEach((col) => {
        if (selectedColumns[col.key]) {
          dados[col.label] = motorista[col.key] || 'N/A';
        }
      });
      return dados;
    });
  ...
  const gerarRelatorioPDF = () => {
    ...
    if (dadosRelatorio.length === 0) {
      alert('Não há motoristas para gerar o relatório.');
      return;
    }

    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text('Relatório de Motoristas', 14, 22);
  ...
  const gerarRelatorioXLS = () => {
    ...
    XLSX.writeFile(workbook, 'relatorio_motoristas.xlsx');
    handleCloseMenu();
  };
  ...
  const gerarRelatorioCSV = () => {
    ...
    link.setAttribute('download', 'relatorio_motoristas.csv');
  ...
  return (
    <>
      <Button
        ...
        disabled={loading || !motoristas || motoristas.length === 0}
        endIcon={<ConfigIcon />}
      >
        Gerar Relatório
      </Button>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <FormControl fullWidth>
              ...
              >
                <MenuItem value="completo">Todos os Motoristas</MenuItem>
                {/* Se quiser pode adicionar filtros extras aqui */}
              </Select>
            </FormControl>
...
export default MotoristasRelatorio;
